...
type Stage = "intro" | "adb" | "bootloader" | "fastbootReady" | "fastboot" | "session" | "unsupported" | "betaCode" | "activating" | "ready" | "risk" | "install";
...
type Release = { manifest?: { version?: string; artifacts?: Array<{ id: string; kind: string; component?: string }> }; profile?: { id?: string }; downloadUrls?: Record<string, string> };

export function Wizard({ developmentHardwareFixture, compatibilityCheckerOnly, betaBrowserInstaller }: { developmentHardwareFixture: boolean; compatibilityCheckerOnly: boolean; betaBrowserInstaller: boolean }) {
  ...
  async function simulateStockDevice() {
    ...
    try {
      const completed: WebCompatibilityScan = { ...DEVELOPMENT_FIXTURE_COMPATIBILITY, deviceId: DEVELOPMENT_FIXTURE_DEVICE_ID };
      setScan(completed);
  ...
  function reviewInstallationReadiness() {
    if (!licenseId || !installerToken || confirmation !== "ERASE PSG1" || !riskAccepted) return;
    setStage("install");
  }
  ...
  return <section className="wizard-card">
    ...
    {stage === "ready" && <div className="success"><strong>✓ Beta release authorized</strong><p>Release {release?.manifest?.version ?? "authorized"} is signed for this PSG1. Aurora Store and RetroArch are included as verified post-flash APKs when present in the signed manifest.</p><button className="button primary wide" onClick={() => setStage("risk")}>Review irreversible installation</button></div>}
    {stage === "risk" && <div className="wizard-step"><Step number={4} title="Irreversible installation" /><div className="warning"><strong>No echOS recovery image is provided.</strong><p>This process erases all data, leaves the bootloader unlocked, may make restoration impossible, and can leave the device unusable. Keep your Discord support ticket open.</p></div><label className="checkbox"><input type="checkbox" checked={riskAccepted} onChange={(event) => setRiskAccepted(event.target.checked)} /> I understand and accept the irreversible beta risks.</label><label className="field">Type <b>ERASE PSG1</b> to start<input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoComplete="off" /></label><button className="button danger wide" disabled={!riskAccepted || confirmation !== "ERASE PSG1"} onClick={reviewInstallationReadiness}>Review installation readiness</button></div>}
    {stage === "install" && <div className="pending"><strong>Beta installation is still fail-closed.</strong><p>The signed release is ready, but no destructive boundary was recorded and no command was sent. Browser flashing remains unavailable until the mirrored image has passed the required Fastboot download-window and stock-hardware validation for this exact profile.</p><small>Do not unlock, flash, or use generic Fastboot commands outside the supervised beta procedure.</small></div>}
