...
export default function Docs() {
  ...
  return <main className="prose">
    ...
    {!compatibilityCheckerOnly && <>
      <h2>{earlyAccessFree ? "Free forever" : "License and checkout"}</h2>
      <p>{earlyAccessFree ? "After the scan matches a signed compatibility profile, Revive activates free device-bound access and issues short-lived installer authorization. No wallet, payment, transaction, or promo code is required—access is free forever. Donations are optional and never affect access." : "After the free scan matches a signed compatibility profile, a compatible Solana wallet authorizes checkout. After finalized payment, the receipt wallet unlocks short-lived web-installer access. The permanent entitlement binds to the cross-checked PSG1 identifier—not the wallet or computer."}</p>
    ...
    <p>{compatibilityCheckerOnly ? "When browser unlock opens, reconnect the physical PSG1 and run the scan again to recover an existing entitlement." : <>Reconnect the physical PSG1 and run the scan to recover an existing entitlement. A factory reset, OS reinstall, or different computer does not consume a second device activation.{earlyAccessFree ? " No original wallet is needed." : " Device-bound recovery credentials remain the long-term fallback if the receipt wallet is later unavailable."}</>}</p>
